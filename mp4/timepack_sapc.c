...
int starttimer()
{
  ...
  int dcount = inpt(TIMER0_COUNT_PORT);
  dcount |= (inpt(TIMER0_COUNT_PORT) << 8);

  startcount = COUNTS_PER_TICK * tickcount + (COUNTS_PER_TICK - dcount);
  timer_running = TRUE;
  return T_OK;
}

/* temporary crude way: better to read the counter and add in the dcounts
 * (converted to us) since the last tick, and similarly change starttime 
 * to record the starting dcount, and adjust for another partial tick.
...
int stoptimer( int *interval )
{
  ...
  *interval = COUNTS_PER_TICK * tickcount + (COUNTS_PER_TICK - dcount);
